...
import { appToday, daysFrom, formatLabel, splitYmd, APP_TIMEZONE } from "../utils/dates";
...
export default class ReservationChecker {
  ...
  async initializeBrowserlessChrome(token) {
    try {
      ...
      // 60000 with a 400, so the check must finish within 60s.
  ...
  async login() {
    try {
      ...
      await this.page.goto(amenityUrl, { waitUntil: "domcontentloaded" });

      // Wait for login form
      await this.page.waitForSelector(
        'input[type="text"], input[name="email"], input[id*="email"]',
        {
          timeout: timeouts.waitForSelector,
        }
      );
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, timeouts.betweenActions)
      );
    } catch (error) {
      ...
      if (moreLink) {
        ...
        try {
          ...
          if (isVisible) {
            ...
            await new Promise((resolve) =>
              setTimeout(resolve, timeouts.betweenActions || 2000)
            );
            return true;
          } else {
            return false;
   ...
   */
  getNext7Days() {
    const start = appToday();

    return daysFrom(start, WINDOW_DAYS).map((ymd) => {
   ...
   */
  async robustBrowserOperation(operation, options: { maxRetries?: number; requiresLogin?: boolean } = {}) {
  ...
  async checkAvailability() {
    try {
      ...
      let allReservations;
      if (process.env.NODE_ENV === 'production') {
        allReservations = await this.robustBrowserOperation(
          () => this.loadAllReservations(),
          { maxRetries: 3, requiresLogin: true }
        );
      } else {
      ...
      console.log("========================================");
      console.log(`Checked at: ${new Date().toLocaleString("en-US", { timeZone: APP_TIMEZONE })}`);
