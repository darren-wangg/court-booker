...
async function startWebhook() {
  ...
  console.log(`🌐 Cloud Environment: DigitalOcean VPS`);
  ...
  try {
    ...
    // Add a keep-alive for the server
    const serverKeepAlive = setInterval(() => {
      // This ensures the event loop stays active
      if (global.webhook && global.webhook.server) {
        console.log('💓 Server keep-alive - server is active');
      }
    }, 60000); // Every minute
    
    global.serverKeepAlive = serverKeepAlive;
...
process.on('SIGINT', async () => {
  ...
  if (global.serverKeepAlive) {
    clearInterval(global.serverKeepAlive);
  }
  process.exit(0);
});
